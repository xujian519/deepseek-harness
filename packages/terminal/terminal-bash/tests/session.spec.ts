...
describe('LocalPtySession readiness and output', () => {
  ...
  it('answers PSReadLine cursor-position queries so pwsh can render its next prompt', async () => {
    ...
    terminal.emitData('\x1b[6n')
    await vi.advanceTimersByTimeAsync(0)
    // The reply reports the emulator's tracked cursor: the readiness prompt
    // `dsh> ` left it on row 1, column 6.
    expect(terminal.writes).toEqual(['Write-Output hi\r', '\x1b[1;6R'])
  ...
  it('answers a cursor-position query split across output chunks', async () => {
    ...
    await vi.advanceTimersByTimeAsync(0)
    // The leading \r\n moved the tracked cursor to row 2, column 1.
    expect(terminal.writes).toEqual(['echo x\r', '\x1b[2;1R'])
    terminal.emitData('x\r\n\x1b]133;D;0\x07dsh> ')
    await vi.advanceTimersByTimeAsync(10)
    expect((await operation.done).waitReason).toBe('stdin_read')
  })
