...
随包的只是索引：条号与主题词取自本仓库既有的引用，全部 `text` 与 `verifiedOn` 仍为 null，且文件头记录第 62/69/70 条一组疑为 2020 年修正前的编号。在人工按官方文本逐条转录之前，对它的每一条引用都报「未核验」——永不会报「已核验」。
...
- **随包索引不是法条文本。** 它只有条号与主题词；在人工按官方来源逐条转录前，每个条目都是「未核验」。因此默认部署下引用始终报「未核验」——这是设计行为，不是"检查通过"。
- **《专利法》索引里第 62/69/70 条一组疑为 2020 年修正前的编号**，与本仓库他处使用的 2020 年编号（第71条损害赔偿、第74条三年时效）不一致。文件头已记录；核验前不得把本索引当作条号权威。
...
- **自动门禁仍保留自己的表。** 专利 preset 已挂载本插件，模型会先跑 law_verify；让工作流质量门禁的静态主题表与规则资产的条号上限改读本索引属后续工作——委派给一条尚未转录的条目，只会把今天的主题检查降级成告警。
