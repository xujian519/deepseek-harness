...
- **`SessionStart` is partial** — JSON `additionalContext` is consumed and gated into the first step, but plain stdout context, `initialUserMessage`, `sessionTitle`, `watchPaths`, `reloadSkills`, and `CLAUDE_ENV_FILE` are unsupported. The payload omits optional fields such as `model`, `agent_type`, and `session_title`.
...
- **`Stop` is partial** — blocking forces another model turn, `stop_hook_active` is `true` when the previous Stop hook already forced a continuation, and `maxStopContinuations` caps consecutive forced continuations before cancelling the run. `last_assistant_message`, `background_tasks`, and `session_crons` are still omitted.
- **Common payload and output fields are partial** — mapped event payloads omit `prompt_id`, `permission_mode`, and `effort` where Claude Code would provide them, and `transcript_path` is never populated: it is always the empty string, because the persistence seam exposes no artifact paths and the default Zstandard-compressed session log is not readable by hook scripts. `systemMessage` is logged + warned but not surfaced; `suppressOutput`, `stopReason` other than as a halt reason, and `terminalSequence` are not applied.
...
The remaining deferred gap is per-session hook-config discovery; the official Claude Code reference is the baseline for closing it.
