...
- **脱离存储的结果。** 所有返回的 header、事件与记录都是克隆；不暴露实时状态，也不保留订阅。源解析本身只借用、不复制，因此一次读取付出的代价取决于它返回的内容，而不是它读过的日志规模。
...
`SessionCorpus` 通过 fiber 绑定可选的 `ctx.sessionPersistence`，并实时优先解析每次读取：已知实时目标直接借用，不查询持久化；否则先用一次单会话观察按 id 定位目标，再通过短生命周期的读取句柄完整读出其日志，并在返回结果前重新检查是否出现实时挂载。解析过程不复制任何事件：实时持有方暴露自身已冻结的日志快照，存储日志要么由后端冻结，要么作为独立持有的事件交出，因此每个端点只克隆自己返回的值。写入者在轮次中途崩溃的冷日志用 `interruptedTurnClosers` 在内存中补齐——读取从不修改持久化。观察到的 header 与加载到的 header 之间会断言兼容性。批量标题读取执行一次元数据列表与有界并发读取，把逐会话失败隔离，而取消会拒绝整个批次。
