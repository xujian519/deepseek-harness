 ...
 *
 * 条号按《专利法》2020 年第四次修正（共 82 条）与《专利法实施细则》2023 年第三次修订：
 * 三项抗辩在 2020 年修正里从 62/69/70 移到 67/75/77，第 62 条改为强制许可使用费裁决；
 * 分案申请在 2023 年修订的细则里是第 48 条。条号判错会把旧号判成正确引用。
 * 《专利审查指南》不在本表内，指南引用因此一律放行。
 */
const PATENT_LAW_TOPICS: Record<string, { max?: number; topics: Record<number, string[]> }> = {
  专利法: {
    ...
    topics: {
      ...
      62: ['强制许可', '使用费', '裁决'],
      67: ['现有技术抗辩', '公知技术抗辩', '不构成侵权'],
      75: ['不视为侵权', '权利用尽', '先用权', '临时过境', '科研实验'],
      77: ['合法来源', '善意使用销售', '不承担赔偿责任'],
    },
  },
  专利法实施细则: {
    topics: {
...
/**
 * invalidationGrounds：可作为无效宣告理由的实体条款，取自《专利法实施细则》2023 年
 * 修订第 69 条第 2 款列举的专利法条号（2、19.1、22、23、26.3、26.4、27.2、33 与 5、9、25）。
 */
const INVALIDATION_GROUNDS = new Set([2, 5, 9, 19, 22, 23, 25, 26, 27, 33])
