...
Function plugin porting the Sati academic-literature layer into the DeepSeek Harness: four free, keyless scholarly sources — arXiv, OpenAlex, Semantic Scholar, and Crossref — normalized behind one connector registry and exposed to the model as two stateless tools, `paper_list_sources` and `paper_search`. The port preserves the source's per-host polite rate limiting (arXiv 1 request per 3 seconds, keyless Semantic Scholar 1 request per second) and its in-process GET cache with LRU eviction, so fan-out across sources is not over-serialized and malformed responses never poison the cache.

## Tools

The model discovers available `db` ids through `paper_list_sources`, then queries a specific source through `paper_search`; the tool count stays constant (two) no matter how many sources are wired in. Both tools are stateless queries over public sources and need no agent or session.
...
Two registered tool definitions: `paper_list_sources` (optional `domain`) and `paper_search` (required `db` and `query`, optional `limit`). Each description instructs the model to call `paper_list_sources` first to discover a `db` id, and notes that fielded arXiv queries such as `ti:transformer AND cat:cs.LG` pass through. Connector enablement changes only which `db` ids are valid, never the definitions.
...
- **No fetch-by-id tool yet** — the connectors implement `fetch` internally, but the first-tier surface exposes only `paper_list_sources` and `paper_search`.
