...
browser 组负责模型可见下载与抓取工具背后的浏览器自动化后端。它提供级联抽象——ego lite、BrowserOS neo、browser-use 与 @playwright/mcp——含能力探测、冷决策路由与浏览器兜底路径的链接提取执行，自 Sati 浏览器后端层（`src/browser/backend/`）移植。`browser-backend/` 负责后端类型、探测、级联路由与链接提取器；各包契约由子 README 负责。
