...
const [REPO_OWNER, REPO_NAME] = (process.env.GITHUB_REPOSITORY ?? `${config.organization}/${config.repository}`).split('/')
if (!REPO_OWNER || !REPO_NAME) throw new Error('GITHUB_REPOSITORY 格式无效')
const PROJECT_ORG = config.projectOrganization
...
async function issueSnapshot(number, status = undefined) {
  const issue = await api(`/repos/${REPO_OWNER}/${REPO_NAME}/issues/${number}`)
  if (issue.pull_request) return null
  const values = await api(
    `/repos/${REPO_OWNER}/${REPO_NAME}/issues/${number}/issue-field-values?per_page=100`,
  )
...
async function upsertAudit(number, errors) {
  const comments = await api(
    `/repos/${REPO_OWNER}/${REPO_NAME}/issues/${number}/comments?per_page=100`,
  )
  ...
  if (errors.length === 0) {
    if (existing) {
      await api(`/repos/${REPO_OWNER}/${REPO_NAME}/issues/comments/${existing.id}`, {
        method: 'DELETE',
      })
    }
    return
  ...
  if (existing) {
    ...
    await api(`/repos/${REPO_OWNER}/${REPO_NAME}/issues/${number}/comments`, {
      method: 'POST',
      body: JSON.stringify({ body }),
      headers: { 'Content-Type': 'application/json' },
    })
...
async function pullRequestSnapshot(number) {
  const [pull, reviewRequests, reviews] = await Promise.all([
    api(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${number}`),
    api(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${number}/requested_reviewers`),
    api(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${number}/reviews?per_page=100`),
  ])
