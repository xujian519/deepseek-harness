...
- **第 1 项（`patent-teams` 守卫去重）：不可行，已回滚。** `state.ts` 与 `invariant.ts` 里逐字重复的 `isOptionalString` 看似可抽，抽成 `src/guards.ts` 后本地全绿（295 用例、`lint`、`typecheck`、各 `verify-*` 均过），但 CI 的 `node 24 / hygiene` 失败：该包从 `lib/index.js`（`.`）与 `lib/invariant.js`（`./invariant`）两个入口发布，而 `files` 是显式清单 `["lib/index.js","lib/invariant.js","lib/types/**/*.d.ts"]`；模块被两个入口同时可达后 tsdown 将其提升为**内容哈希分块**，构建出的 `lib/invariant.js` 首行引用 `./guards-DfhXKZCW.js`，而 `scripts/verify-built-package-invariants.mjs` 只暂存清单声明的 `lib/` 文件再加载伴随模块（其源码注释即写明此用途），故必失败。**该重复是承重的**：invariant 伴随必须构建成自包含入口。要让去重成立，只能把哈希分块通过 glob 写进 `files`（需一并动约束表），或让伴随模块从包入口 `./index.js` 导入（`interaction/permission-presets` 与 `interaction/user-approval` 的既有做法）——两者代价都高于省下的 5 行。该 PR 已关闭，机制与证据见其关闭说明。
