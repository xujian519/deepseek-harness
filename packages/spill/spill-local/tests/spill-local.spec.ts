...
describe('startup cleanup sweep', () => {
  ...
  it('keeps a file exactly at the boundary (only strictly-older expires)', async () => {
    const dir = sessionDir(root, 'sess-1')
    mkdirSync(dir, { recursive: true })
    // Pin the cutoff to a whole second: utimesSync takes seconds, and a
    // fractional epoch second loses sub-millisecond precision in the
    // seconds→nanoseconds round-trip, which would age the file a hair past
    // the cutoff instead of exactly on it.
