...
- The fork's CI wall-clock grows by the coverage aggregate, which is the longest job in the workflow: its first run measured 20 minutes on the 4-core runner with the original four-process budget, and it runs only on pull requests. The reduced budget below trades some of that wall clock back for per-process headroom.
- The lane's first two runs failed on load-sensitive cases rather than on the gate: run 1 cleared every test and reported the `pty-manager.ts` threshold miss, and run 2 — on code that fixes that file — reported zero threshold misses, having instead tripped two cases that passed in run 1: `packages/experimental/ptc-runtime-python/tests/runtime.spec.ts` hitting the 60 s wall-clock ceiling its own comment budgets for "the coverage lane's V8 instrumentation with several workers sharing a box", and a `packages/preset/agent-presets/tests/mount.spec.ts` mount assertion whose two refusal paths race. Four instrumented processes on four cores is a heavier posture than either upstream's six on sixteen or the fork's uninstrumented three, so `DSH_COVERAGE_MAX_WORKERS` dropped to two. A third run is what confirms the posture; a further load-sensitive failure would move the work to the owning tests rather than to this budget.
