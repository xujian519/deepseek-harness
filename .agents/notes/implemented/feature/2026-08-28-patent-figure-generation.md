...
- `figure/svg-annotate.ts` — post-process existing SVGs with ` (numeral)` at matching `<text>`/`<tspan>`; rejects entity definitions/CDATA and oversized input (Graphviz's standard `<!DOCTYPE svg>` is allowed), reports unmatched references as warnings.
