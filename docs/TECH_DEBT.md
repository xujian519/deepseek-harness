...
## 2026-09-23 更新（专利域机械收敛批次的落地状态）

- **批次登记形态（防止后续追错对象）**：2026-09-21 专利域审阅清单（`.agents/audits/2026-09-21-patent-domain-review-manifest.md`）里的 `### Issue #172`–`#186` 是**计划编号，未提交为 issue**。本仓 issue 与 PR 共用编号空间，当前 issue 最大号为 **171**，`area/patent` 下只有 #7–#11（均已关闭）；#185 那个号在真实 tracker 里是一个已合并的 document-studio 修复 PR。因此该批次的权威记录只有清单本身与本节，不要按清单编号去 tracker 找对应 issue。
- **第 1 项（`patent-teams` 守卫去重）：机制已查清，但当时误判为不可行**（见本条目末尾的修正）。 `state.ts` 与 `invariant.ts` 里逐字重复的 `isOptionalString` 看似可抽，抽成 `src/guards.ts` 后本地全绿（295 用例、`lint`、`typecheck`、各 `verify-*` 均过），但 CI 的 `node 24 / hygiene` 失败：该包从 `lib/index.js`（`.`）与 `lib/invariant.js`（`./invariant`）两个入口发布，而 `files` 是显式清单 `["lib/index.js","lib/invariant.js","lib/types/**/*.d.ts"]`；模块被两个入口同时可达后 tsdown 将其提升为**内容哈希分块**，构建出的 `lib/invariant.js` 首行引用 `./guards-DfhXKZCW.js`，而 `scripts/verify-built-package-invariants.mjs` 只暂存清单声明的 `lib/` 文件再加载伴随模块（其源码注释即写明此用途），故必失败。**该重复是承重的**：invariant 伴随必须构建成自包含入口。要让去重成立，只能把哈希分块通过 glob 写进 `files`（需一并动约束表），或让伴随模块从包入口 `./index.js` 导入（`interaction/permission-presets` 与 `interaction/user-approval` 的既有做法）。该 PR 已关闭，机制与证据见其关闭说明。**修正（同日）：上述"不可行"结论过于绝对。** 全仓实测确认，本仓对分块的做法就是**把分块用 glob 写进 `files`**——`ssh`（`lib/protocol-*.js`、`lib/schemas-*.js`、`lib/stream-security-*.js`）、`subprocess-local`（`lib/runner-*.js`）、`sandbox-windows-acl`（`lib/types-*.js`）、`webworker-packer`（`lib/repository-*.js`）四个包均如此；且分块的触发条件是「某模块获得了新的入口可达性」，而非「跨入口」本身（`webworker-runtime` 的 `image-layout.ts` 跨三入口可达却不分块）。因此该项**可行**：抽件 + 在同包 `files` 加一行 `lib/guards-*.js`。当时未做全仓产物比对就下了否定结论，属于判断过早。**最终处置（同日）：不重做**——该路线（把哈希分块写进 `files` 并登记约束表）已撤回，#195 保持关闭，两份守卫体与 `isNonBlankString` 的归属按现状保留。
- **第 4 项（`methodology` 的 8 个同构 `identify`）：已落地。** 8 个组件的 `identify` 全部是 `return keywordScore(context, TRIGGERS)`。清单建议的「提升为 `MethodologyComponent` 的默认实现」在 TS 里不可行（接口不带实现），故改为在 `runtime/keywordMatch.ts` 导出 `keywordIdentify(triggers)` 供各组件一行接线，`MethodologyComponent.identify` 保持为必需成员（不弱化契约）。46 用例全绿，`hygiene` 通过。
- **第 3 项（三份 `asset-location`）：建议不落地。** `patent-deadline` 与 `writing-patterns` 两份各 30 行、仅常量名与函数名不同，`patent-rule` 那份 72 行是同一契约的扩展版；真正可复用的只有承重的 3 行（显式覆盖优先，否则 `fileURLToPath(<assets URL>)`）。共享归属地需要一个新包或新增跨包依赖边，会连动 manifest、lockfile、依赖目录与模块图，收益只有个位数行数；清单同时要求的「候选数取决于模块在 `src/` 下的深度」属文档补充，可单独做。
- **第 2 项（两处闭合联合改共享 `assertNever`）：需先决策。** `patent-core` 的 `graph/merge.ts` 手写 `never` 检查抛的是领域错误 `GraphMergeError`（该类型在 `fail_on_conflict` 可达路径上确需保留），换成共享 `assertNever` 会改不可达路径上的错误类型与消息文本；`checker/engine.ts` 那处虽抛普通 `Error`，消息文本同样会变——两者均属行为改动，清单建议的「`assertNever` 之后补一行转换」不可达（`assertNever` 返回 `never`）。
- **第 6 项（注释清扫）：已落地。** 三处「从哪来、以后要搬到哪」的叙述改为现状陈述：`patent-data/src/ego-session.ts` 与对端 `browser/browser-backend/src/ego-backend.ts` 的 `jscpd:ignore` 理由（两侧同改，避免不对称）、`patent-rule/src/asset-location.ts` 的模块文档、`patent-rule/src/runtime/patent-compliance.ts` 的清单注释（保留「上游生成物逐字镜像」这一再同步锚点）。
- **本轮验收**：受影响包 `vitest run`（`patent-data`、`patent-rule`、`browser-backend`）+ `pnpm run lint` + `pnpm run test:docs` + `pnpm run doc-sync`（改动含代码注释与本节台账）。

## 2026-09-23 更新（代码精简扫描：Day 5–8 候选以「受理重复」结案）

- **背景与实测事实**：为期 10 天的全仓精简扫描（范围 `packages/*/src`）在 Day 5–8 累计筛出约 15 处 `assertNever` 本地副本与约 10 处手写 record 守卫，它们全都卡在同一条约束上——所在包未声明原语归属包（`assertNever` 在 `@deepseek-ai/dsh-util-values`，`isRecord`/`asRecord` 在 `@deepseek-ai/dsh-value`）。而 351 个包中已有 **81 / 82 个**声明这两个原语包，且 `dsh-value` 自身就依赖 `dsh-util-values`：因此这些站点属**个别包漏声明**，不是"刻意不依赖"。
- **成本核算与结论**：补边需逐包改 `dependencies`（Host 面/Client 面规则由 `verify-package-dependencies` 把关）、刷新 `pnpm-lock.yaml`、重生成依赖目录，并按包跑 `build` + `hygiene`；而收益是每处 1–3 行。**结论：受理这些重复，不补依赖边。**
- **站点计数校正（开工前核对发现）**：按**函数体形状** grep 会过度命中——8 个 record 站点中 3 个是**具名/域名守卫**（`isSessionFormatJsonObject`、`releasedIsRecord`、`isHeaderLine`），它们收窄到领域类型而非 `Record<string, unknown>`，属该领域的词汇，本不计入可换站点；真正可换的内联检查只有 5 处。**判据：判定可换站点必须看声明形态（具名 + JSDoc + 收窄到领域类型 ⇒ 保留），不能只看函数体。**
- **两处独立排除（行为变更）**：`typert/generator/src/renderer.ts` 的本地断言抛领域错误类 `TypeGraphRenderError`、`compaction/command-compact/src/index.ts` 的本地断言抛 `TypeError`（均在 `instanceof` 下可观测），改共享助手属行为变更，未做。
- **已交付的同族去重（不受本条结论影响）**：`experimental/inspector` 一个包内 10 处 `assertNever` 收敛为共享助手（该包本就声明了 `dsh-util-values`）、`patent-core` 两处闭合联合改用共享 `assertNever`、`plan-mode` 确认文案单点归属、客户端 4 项死面删除、`webworker-runtime` 虚拟根由 `image-layout.ts` 单点派生。
- **本条记录的用途**：后续扫描**不再重新报出**这些站点；若将来某个包因其他原因需要声明这两个原语包，可在同一次改动里顺带收敛其站点。
