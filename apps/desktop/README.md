...
6. Persist each next activation phase before its directory move, move the active profile to `$DSH_HOME/desktop/rollback/profile`, and move staging into `$DSH_HOME/profiles/desktop-runtime`. Recovery combines the journal with the actual profile, rollback, and staging directories, so interruption in either write-to-move gap restores or retains a complete profile.
...
`DSH_DESKTOP_UNSIGNED_BUILD=1` skips the macOS seed signing, notarization, and post-sign verification, so the target emits an ad-hoc application instead of a release. The identity and Team ID inputs are still read, but the artifact carries no Developer ID signature and no notarization ticket: Gatekeeper refuses it on other machines, and it is never a shippable release. Release packaging keeps requiring the signing and notary inputs above.
