...
Surface events (`user/message`, `assistant/message`, `tool/result`) must declare how they join the ordered surface. An Assistant message embeds the exact compact provider stream that produced it; `assistant/attempt`, boundaries, and other log-only events never produce a message. Non-surface events accept an optional `{ ignorable: true }` marker so out-of-repo plugin telemetry can be safely skipped by readers that do not know its type.
