...
| `ctx.pluginMarketController` | `core` | [`api-plugin-market-controller`](../packages/api/plugin-market-controller) | - | - | - | 对 plugin-market 发现接缝（源列表、搜索、预览）的只读 Remote 投影；安装与卸载仍归 profile CLI。 |
...
| `ctx.promptCache` | `seam` | [`prompt-cache`](../packages/core/prompt-cache) | - | [`system-prompt`](../packages/core/system-prompt) | - | 可选组装前缀缓存：system-prompt 通过 `ctx.reflect.get("promptCache", false)` 读取；未挂载策略时组装逐字节走既有路径。 |
...
| `ctx.selfEvolve` | `seam` | [`self-evolve`](../packages/self-evolve/self-evolve) | [`self-evolve-basic`](../packages/self-evolve/self-evolve-basic) | [`tool-self-evolve`](../packages/self-evolve/tool-self-evolve) | - | 提供方拥有触发策略、基于验证器的失败挖掘、验证（fork 重放、held-out 搜索、LLM judge）与可逆的 L1/L2 提交；tool-self-evolve 是面向模型的控制器，用于检查模式并启动显式循环。 |
| `ctx.selfEvolveBenchmark` | `seam` | [`self-evolve-benchmark`](../packages/self-evolve/self-evolve-benchmark) | - | - | - | 一个互补的自进化维度：对目标 agent 在持久化基准上打分，并在严格的 improve-or-rollback 策略与整版快照版本化下进行优化；评估与优化通过 fork 子代理执行，私有 rubric 永不进入优化器上下文。 |
...
| `ctx.desktop` | `seam` | [`desktop-seam`](../packages/desktop/desktop-seam) | [`desktop-shell`](../packages/desktop/shell) | [`desktop-directory-picker`](../packages/desktop/directory-picker) | - | Electron 主进程拥有原生对话框、通知、菜单、全局快捷键、托盘与拖放；shell 提供方通过本地 socket 将它们桥接到后端，使渲染进程保持沙箱化。 |
...
| `ctx.betterSidebar` | `seam` | [`better-sidebar`](../packages/client/better-sidebar) | - | - | - | 以 ctx.betterSidebar 发布的 Service Definition：client 插件注册侧边栏标签页类型与文件查看器；better-sidebar 面板是按会话渲染已注册类型的 provider 宿主。 |
