...
| 负载 | 一条 `turn/start` 加 45,000 条 745 字节的 `user/message`（`HISTORY_READ_TEXT_BYTES`，门禁会断言每一轮都是这一尺寸）：45,001 个事件、33.5 MB 各不相同的用户文本。每轮文本都不同，因此存储日志占用真实内存，而不是共享同一个字符串。整份日志始终处在同一个未闭合的 turn 内：已发布格式的关系校验器只在一个 turn 未打开时才接受 `turn/start`，因此每轮一个 `turn/start` 而没有配对的 `turn/end` 会被判为损坏日志 |
...
| 判定 | 见下表。两个时间预算取自标准双核托管 CI 中位数（向上取整）乘以波动余量；瞬时堆预算沿用参考机期望值，只使用波动余量 |
...
负向控制：把被移除的复制放回 `borrow` 即恢复改动前的分配量，门禁在所有非时间用例上都会拒绝它——两个瞬时堆预算，以及两个受限堆完成检查（`SIGABRT`、`Reached heap limit ... JavaScript heap out of memory`）。两个时间预算是按实际执行它们的标准双核托管 runner 标定的 CI 信号，共享的 2× 时间倍率低估了该机器：run 35981330076 的 job 107573671699 报告 `readEvent` 为 432.7、455.8、440.1、426.3、444.5 ms，`readSurface` 为 764、770.9、752.7、773.5、778.1 ms，因此两个预算改为缩放向上取整后的 450 ms 与 800 ms 托管期望值，而不是参考机中位数。同一次运行测得相对参考机的托管倍率为 2.5×，据此把记录在案的参考机中位数带过两个预算（313.3 ms → 783 ms，458.5 ms → 1,146 ms）；门禁的校准用例断言这一点，取代参考机上的一次运行。本门禁不覆盖：Gateway 网络传输、Client fold、浏览器 paint、模型时延。负载也只含一个存储会话，因此门禁不测量读取此前执行的整库遍历；该移除由本包测试与它自身实测的列举成本钉住。
