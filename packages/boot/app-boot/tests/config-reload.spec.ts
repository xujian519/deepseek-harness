...
describe('loader entry id collisions', () => {
  it('keeps the last good tree when a nested row reuses its containing group row id', async () => {
    ...
    // at full CPU. The pre-apply validation turns that shape into a failed
    // reload before any entry is created or adopted. `refresh()` reports a
    // failed reload as a warning rather than a rejection — a hot reload of a
    // live app keeps the running tree — so the validation failure is asserted
    // through the logged reload-failure line, not through a thrown error.
    const { ctx, dir, include } = await bootTree('- id: noop\n  name: ./noop.mjs\n')
    const warnMessages: unknown[][] = []
    ctx.logger.exporter({
      levels: { default: LoggerLevel.WARN },
      export: (message) => {
        if (message.type === 'warn') warnMessages.push(message.args)
      },
    })
    try {
      writeFileSync(join(dir, 'cordis.yml'), [
      ...
      await expect(include.refresh()).resolves.toBeUndefined()
      const logged = warnMessages.flat().map(value => String(value)).join('\n')
      expect(logged).toContain('config reload at %C failed; keeping the running tree')
      expect(logged).toContain('duplicate loader entry id: grp')
