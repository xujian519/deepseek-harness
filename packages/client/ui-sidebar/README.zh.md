...
外壳会提升外部的个人工作台入口：把 `button[data-dsh-personal-workbench-entry]` 放到 New Session 紧后一格的槽位，赋予它 New Session 的类、标签类与图标尺寸（展开列 14、轨道 18），并隐藏它所占槽位对应的定时任务触发器（`button.dshc-trigger`）。插件把自己的入口样式表在 bundle 之后注入文档，因此该表面样式来自 `SidebarRoot.module.css` 中同时命中该类与该入口属性的规则；入口自身的 hover 与选中态与该规则同权重，仍会生效。
...
- **定时任务触发器会被提升逻辑隐藏**：在该外壳里它是该面板的唯一入口，因此提升逻辑运行期间无法从侧边栏打开该面板。
