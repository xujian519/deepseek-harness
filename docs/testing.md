...
- **Built-artifact lane** (`pnpm run test:built`; required fork PR gate `node 24 / built artifacts`): the lane builds first and runs the suites the unbuilt unit lane silently skips, warning by name for any that still report no pass ([lane rule](../scripts/built-artifact-suites.ts)).
