...
The workflow quality gate's static table and `patent-rule`'s `citation_analysis` ceiling are **not** rewired to the index in this change. The gate decides with a five-verdict vocabulary of its own (`valid` / `unknown` / `unverifiable` / `suspect` / `invalid`), `unknown` passes, and its table still carries the 2008 revision's article numbers the index has now moved past — so delegating means deciding what the gate does with each decision this index returns, which is a change to the gate's contract rather than to the index. The duplication stays visible rather than half-fixed.
