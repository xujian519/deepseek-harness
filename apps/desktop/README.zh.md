...
桌面应用是包裹 dsh Web UI 的 Electron 壳。它不打开监听端口：内置的上游 Node.js 子进程启动已安装的 dsh 项目，带版本的分帧字节管道在没有外层 Base64 信封的情况下承载 Fetch 请求与流式响应，Node IPC 承载生命周期控制，`dsh-app://` 则提供与后端版本匹配的客户端资源。壳拥有系统托盘——关闭最后一个窗口会将应用隐藏到托盘——并以 `window.desktop` 向 Web UI 暴露打印 PDF 桥。已安装的后端通过单连接本地 JSON-RPC socket 使用壳侧的 OS 能力（对话框、通知、菜单项、全局快捷键与托盘定制）。
...
应用菜单在壳自有入口之外携带平台 `editMenu`、`viewMenu`、`windowMenu` role，系统级复制/粘贴/撤销加速键与右键编辑对 focused renderer 生效。渲染进程打开的外部 `http(s)` 链接通过 `shell.openExternal` 转交系统浏览器；壳从不打开第二个 `BrowserWindow`。
...
4. 插件添加、更新和删除使用内置 pnpm 及 Desktop 独有的包管理器状态。保留的宿主包必须声明为 peer；共享包的嵌套副本和别名会被验证拒绝；比较接受预发布版本，因此预发布宿主发行能满足覆盖它的预发布 peer 范围。普通插件依赖必须解析到 profile 内部。
...
正常打包只需执行一条完整命令。该命令会先准备发布资源，再生成宿主平台的安装包与更新元数据。所有目标都要求通过 `DSH_DESKTOP_APP_ID` 提供反向域名形式的应用 ID。默认发布保留官方 `DeepSeek Harness` 名称与 `apps/desktop/assets` 内置的品牌图标；设置 `DSH_DESKTOP_PRODUCT_NAME` 为应用指定品牌名称，设置 `DSH_DESKTOP_ICON_DIR` 指向包含 `icon.icns` 与 `icon.ico` 的目录以替换内置图标。macOS 目标还要求通过 `DSH_DESKTOP_MACOS_SIGNING_IDENTITY` 提供 electron-builder 证书限定名，通过 `DSH_DESKTOP_MACOS_TEAM_ID` 提供对应的 10 字符 Apple Team ID，并提供一套完整的 notarytool 凭据方案。App Store Connect API Key 方式使用以下变量：
...
该命令要求设置 `DSH_DESKTOP_APP_ID` 并具备常规构建依赖，包括 Python；Windows 目标还需编译原生模块所需的 Visual C++ 构建工具。Python 不在 `PATH` 中时，将 `PYTHON` 设置为其可执行文件路径。命令将产物写入 `.desktop-build/targets/<target>/unsigned-artifacts/`，省略自动更新配置，清除签名凭据，且不生成发布完成记录。它不需要 EV 凭据或更新源地址，macOS 目标还额外不需要 Developer ID 与公证凭据。签名打包和上传命令仍遵循正式发布要求。

macOS 产物不含 Developer ID 签名与公证票据，在其他机器上会被 Gatekeeper 拒绝；请在构建机上安装。它永远不是可发布的正式版本。
