...
// An internal read size, not config: it bounds one PTY read, and the assembled
// result is the same for any page size.
const SCROLLBACK_PAGE_LINES = 1_000
/**
 * Delay between PTY scrollback reads while a command runs. An internal cadence,
 * not config: a shorter interval only raises host CPU for the same captured
 * output, while a longer one delays the completion marker.
 */
