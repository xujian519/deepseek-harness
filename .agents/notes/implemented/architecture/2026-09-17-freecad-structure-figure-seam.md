...
**Page + minimal template, one `DrawViewPart` per view.** TechDraw only projects a view that belongs to a `DrawPage` carrying a template, so the script writes its own minimal blank SVG template (one invisible page rect) and pins `doc.TransientDir` to an output-directory subdirectory — TechDraw copies the template relative to that property, which FreeCAD leaves empty whenever its own cache directory is unwritable (any sandbox denying `~/Library/Caches/FreeCAD`), resolving the copy to `/` and failing the render; `doc.FileName` does not participate in that copy. Each requested view is a single `TechDraw::DrawViewPart` with explicit `Direction`/`XDirection`; `DrawProjGroup.addProjection(str)` throws `TypeError: wrong type` in 1.1.3 and is avoided. The legacy `Drawing` workbench was removed in FreeCAD 1.1 and is not used.
...
**Success is exit code plus manifest presence only.** `freecadcmd` treats `~/Library/Preferences/FreeCAD` and cache write failures as non-fatal warnings (STEP load, projection, and export all succeed), and on macOS those paths do not follow the redirected HOME — harmless only because the script pins `TransientDir` into the output directory itself, an empty `TransientDir` being fatal to the template copy. Conversely, `freecadcmd` swallows uncaught Python exceptions and exits 0, so the script wraps `main()` and re-raises failures as `traceback.print_exc()` + `sys.exit(1)`; without that wrapper a failed projection would look like success. stderr text is therefore never a failure criterion; it is only quoted into the error message.

**Tool layer reuses the figure invariants.** Output SVGs pass the same `assertSafeSvg` gate as the Graphviz path, callout labels/numerals pass `figureWordingWarnings`, and each figure persists to `figureIndexStore` with `analysis.figureType='structure'` and `modelUsed='freecad-structure'`. A directory `model_path` renders every supported model in sorted order with incrementing figure numbers, each into its own `fig{N}/` subdirectory so manifest/script/template/home files never collide across a batch; that directory form is rejected with `invalid_tool_input` when `callouts` are supplied, since a callout's `point3d` is one model's coordinate and projecting the same 3D anchor onto a different model lands off its geometry. `components` is derived once from `callouts` (not accumulated across figures), so every per-figure index entry carries the same callout components without duplication.
...
- New Config surface: `freecadExecutable`, `structureFigureEnabled`, `structureFigureScale`, `structureFigureViews`; discovery order mirrors Graphviz (override → `DSH_FREECAD_CMD` → platform candidates → `PATH`). `structureFigureViews` is a `z.union` of the supported view names, so an unknown configured view is rejected at config load instead of surfacing later as a misleading `invalid_tool_input`.
- Model-supplied `scale` (finite and > 0) and `figure_number` (positive integer) are validated in the tool layer before rendering: the tool-input schema bounds only their types, so an unchecked value would reach FreeCAD as a degenerate `width="0mm"` SVG or an opaque projection failure.
...
- `tests/figure-freecad-structure-script.spec.ts` — pure-function assertions on the built script (API usage, view table, `TransientDir` pinning, payload round-trip) without FreeCAD.
