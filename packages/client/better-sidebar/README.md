...
The sidebar mounts by one insert row: a composition that adds it gets the panel beside the conversation with no further wiring. The desktop composition inserts that row disabled; a browser `dsh web` composition mounts nothing, so a deployment that wants the panel inserts the row itself.
