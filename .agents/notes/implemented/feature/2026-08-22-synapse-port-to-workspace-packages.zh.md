# Agent Note: Synapse 会话地图插件复刻：宿主/客户端双包接入 web profile
...
- **整包搬运动态插件。** 上游是纯 JS 单包（函数插件 + window.__ModuleLoader__ 客户端插件 + 静态资产），整包搬运最省。否决：本仓库编译面划分（tsconfig.client.json / tsconfig.host.json）不允许一个 packages/client/* 包同时编译宿主侧与浏览器侧会话代码，因此必须拆成宿主/客户端两个一等工作区包。
- **用 live SessionStore 作为画布基线。** 上游用 ctx.sessions.list() 重放。否决：本仓库 web 宿主对恢复的会话保持冷态，启动时 live store 为空，会话列表只是持久化读取；改用 SessionPersistence（revision 键）作为冷态基线，叠加 live 事件流与浏览器 syncSessions 刷新。
- **新会话用 sessions.create。** 否决：create 不在本仓库 ISessions 契约上，改用客户端工作区流转（ctx.workspaces.startSession 复用空白会话 + 当前会话监听）。
...
- 单包 tsc 干净；pnpm run typecheck（两个聚合）通过。
