...
- **Verification concurrency is fixed at two generations** — a verifier is a worker that loads a whole generation, so the bound is a memory ceiling rather than a CPU-sized pool; it is not a `Config` field, and raising it multiplies peak memory by the concurrent generations.
