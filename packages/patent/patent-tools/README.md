...
| `search_patent_figure` | search | keyword retrieval over the figure index (not image-gated) |
| `patent_pdf_download` | document | `@deepseek-ai/dsh-patent-data` ego-browser / fetch |
...
| `knowledge_note_save` | knowledge | storage-backed (knowledge.db write API deferred) |
...
- **Image-modal gate scope** — `analyze_patent_figure` is gated on the resolved figure-model route's declared image input (denied with error code `model_cannot_accept_image` when absent); `search_patent_figure` reads the prebuilt index and is intentionally not gated (matches Sati, which gates analyze only).
...
- **Knowledge note write path** — `knowledge_note_save` writes through the case directory / storage rather than the knowledge.db (which has no write API); a native write API is deferred.
