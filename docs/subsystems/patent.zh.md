...
本页记录三个服务接缝（`ctx.patentData`、`ctx.patentKnowledge`、`ctx.patentWorkflow`）与纯库 ModelPort 契约（`@deepseek-ai/dsh-patent-core`）。截至 2026-08-17，三个服务均处于脚手架阶段：Service Definition 类与 `ctx` 键已存在，方法面随移植模块按计划阶段落地（P1.2 数据、P1.3 知识、P3.1 管线）。模型可见工具、规则门禁与文书渲染位于消费包（`dsh-patent-tools`、`dsh-patent-rule`、`dsh-patent-document`、`dsh-tool-literature`、`dsh-methodology`）。
