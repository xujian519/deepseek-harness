...
- 图谱导航走 `cnlaw_graph_ground(article, law, ipc, k)` 与 `cnlaw_graph_patent(pn)`；`:8001` 的图谱端点不再作为 `curl` 路径提供。
- 案件决策链走 `cnlaw_case_record` / `cnlaw_case_get` / `cnlaw_case_chain` / `cnlaw_case_similar`；`:8001` 的案件端点对不挂桥的部署保持 REST-only，但永远不是优先的并行路径。
...
端点清单仍留在提示词里，用于没有 MCP 面的那些接口——`:8100 /search`、`/search/decisions`、`/search/judgments` 以及 `:8001` 的 IPC 路由——也作为不挂桥部署的可用路径。分工是 `:8001` 的 graph/case/workflow 端点走 MCP，其余走 REST。
...
- persona 随这条纪律的加强而增长。2026-09-21 的利用度审计（同一窗口内 MCP 48 次调用，对比含检索在内的 REST 命令 202 次）促成了案型路由表与"不要用 `curl` 调 `:8001` 图谱/案件端点"的明文措辞；preset 的 persona prefix 块实测 15,999 UTF-8 字节，上限为 `agent-instructions` 的 65,536 字节。
