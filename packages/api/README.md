...
| [`account-controller/`](account-controller/README.md) | Owns the Account Remote surface: authenticated commands and a snapshot stream over the account service, without returning tokens or PKCE secrets. | `ctx.accountController` / `ctx.remote.account` |
...
| [`plugin-market-controller/`](plugin-market-controller/README.md) | Forwards the read-only browser face of the plugin-market seam: source listing, source catalog search, and install preview. | `ctx.remote.pluginMarket` |
