...
`checkEmbodimentCoverage` 生成权项—实施例覆盖矩阵：每条条目给出权利要求标识、其技术特征与支持它的实施例引用，本模块按整串出现判定 full/partial/none。调用方给出的覆盖度判读不是输入——结论只来自这两列事实。断档仅在全部条目编号合法时推断，因为非法条目本应占据的编号未知。特征数按去重后计算，重复特征不再虚增已覆盖数（上游实现按原始列表计数）。
