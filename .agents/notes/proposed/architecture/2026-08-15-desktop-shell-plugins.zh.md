...
- `@deepseek-ai/dsh-desktop-shell` 在 `apply()` 中读取 `DSH_DESKTOP_BRIDGE_PATH` 并连接。
- 协议采用 JSON-RPC 2.0，一个双向通道：
  - 后端 → 主进程：方法调用（`dialog/showOpenDialog`、`notification/send`、`tray/set` 等）。
  - 主进程 → 后端：通知（`shortcut/triggered`、`file/dropped`、`tray/clicked`、`menu/activated`）。
...
- Main 能显示目录选择器、发送通知，并接收来自后端的全局快捷键事件。
- 桥接断开可被处理并被观测到。
