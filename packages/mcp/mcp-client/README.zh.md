...
| `surfaceInstructions` | 两者 | 否 | 将服务器 `instructions`（MCP initialize 响应）表面化为系统提示 section（默认 `true`） |
...
- 服务器 instructions：当服务器在 initialize 响应中提供 `instructions` 时，它们会被注册为 `mcp:<serverName>:instructions` 系统提示 section（order 155，工具指引带）。该 section 在每次组装时重新读取当前世代，因此重连返回不同 instructions 时无需重新注册即可反映；缺失或空值不贡献任何内容。可用 `surfaceInstructions: false` 关闭。
...
| `ctx.systemPrompt` | 将服务器 `instructions` 表面化为提示 section（启用 `surfaceInstructions` 时） |
...
- **只桥接 MCP 的工具能力与服务器 instructions**：资源和提示词没有 harness 消费接口，暂缓实现。
