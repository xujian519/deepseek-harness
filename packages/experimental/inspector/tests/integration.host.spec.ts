...
describe('experimental Inspector real Worker', () => {
  ...
  it('forwards Client Console objects through isolated realm sessions', { timeout: 30_000 }, async () => {
    inspector = await startInspector({ port: 0, captureFetch: false })
    ...
    try {
      await vi.waitFor(observeBoth, { timeout: 4_000, interval: 50 })
    } catch {
      // The `log` RPC was acknowledged by the client worker, so reaching here
