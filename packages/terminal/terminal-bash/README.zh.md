...
一个后端服务两种方言：bash 与 pwsh 共享同一套会话机制——清理器、有界缓冲区、就绪轮询、取消与关闭——只在 argv、环境与提示符安装方式上不同。bash 通过 `PS1` 加 `PROMPT_COMMAND` 接收私有标记。pwsh 会写入提示符函数、固定 UTF-8 控制台编码，并只在后端报告 `stdin_read` 后发布启动；回显的设置文本不能发布 shell。pwsh 中的 PSReadLine 在渲染每个提示符前会向终端查询光标位置并阻塞等待回复，因此会话以标准报告应答这些光标位置查询；bash 中的 GNU readline 从不查询，该应答在那里是无效的。一个不保留 scrollback 的 `@xterm/headless` 实例会消费原始 PTY 数据，并通过同一句柄返回终端协议响应；逐行 sanitizer 仍是唯一输出投影。
...
| [`src/send-lifecycle.ts`](src/send-lifecycle.ts) | 持有终端槽位的那次发送：它的截止时间、取消监听、中断交接与提供方写入 |
...
bash 会在内核发布前台进程组的返回之前打印 `PROMPT_COMMAND`，因此提示符标记可能在更早的一次发送仍占有终端时到达。会话保留这样的标记，而不把它归给正在等待的发送；就绪轮询仍是权威：只有当 bash 持有前台时才接受保留下来的标记。把标记绑定到某个世代（从而使信号延迟到达的提示符永远不会被读作后一次发送的证据）需要复现用例；在此之前，「保留标记 + 轮询为准」就是调用方可以依赖的约定。

就绪证据留在会话上，由会话收集、由会话判定：`promptSeen`、`promptTextSeen` 与保留的 `promptTail` 记录私有提示符标记；`lastOutputAt` 记录输出静默；`shellPgid` 记录 bash 持有的前台进程组，在首个标记到达时学到一次；`initializing` 使启动中的发送在精确等待与静默两档额外要求先有输出。`resetReadinessEvidence` 在准入时以及携带输入的提供方写入之前丢弃标记与输出证据，因此一次发送只按它自己那次写入产生的证据判定；`shellPgid` 与 `initializing` 不随该重置清空。轮询器只负责排定：下一次轮询何时运行，以及是否还可以为持有槽位的发送续约。
