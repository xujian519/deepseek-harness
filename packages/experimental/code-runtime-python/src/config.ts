 ...
 * executable probe.
 * @module @deepseek-ai/dsh-experimental-code-runtime-python/src/config
 */
...
export function resolveRuntimeConfig(config: Config, frameParseCapBytes: number): ResolvedConfig {
  ...
  if (!Number.isInteger(resolved.cpuSeconds)) {
    throw new Error(`dsh-code-runtime-python: config.cpuSeconds must be a positive integer, got ${String(resolved.cpuSeconds)}`)
  }
  ...
  if (!Number.isSafeInteger(resolved.cpuSeconds + 1)) {
    throw new Error(`dsh-code-runtime-python: config.cpuSeconds must be at most ${Number.MAX_SAFE_INTEGER - 1} (it and its +1 hard limit cross to setrlimit as exact integers), got ${String(resolved.cpuSeconds)}`)
  }
  ...
  if (!Number.isSafeInteger(resolved.addressSpaceMb * 1024 * 1024)) {
    throw new Error(`dsh-code-runtime-python: config.addressSpaceMb must be at most ${Math.floor(Number.MAX_SAFE_INTEGER / (1024 * 1024))} (its byte count crosses the wire as an exact integer), got ${String(resolved.addressSpaceMb)}`)
  }
  ...
  if (resolved.pythonBin === '' || resolved.pythonBin.includes('\0')) {
    throw new Error(`dsh-code-runtime-python: config.pythonBin must be a non-empty path without NUL bytes, got ${JSON.stringify(resolved.pythonBin)}`)
  }
  ...
  if (resolved.maxWallMs > MAX_TIMER_DELAY_MS) {
    throw new Error(`dsh-code-runtime-python: config.maxWallMs must not exceed ${MAX_TIMER_DELAY_MS} (setTimeout clamps a larger delay to 1ms), got ${String(resolved.maxWallMs)}`)
  }
  if (resolved.graceMs + CLOSE_REAP_MARGIN_MS > MAX_TIMER_DELAY_MS) {
    throw new Error(`dsh-code-runtime-python: config.graceMs must not exceed ${MAX_TIMER_DELAY_MS - CLOSE_REAP_MARGIN_MS} (its close deadline adds ${CLOSE_REAP_MARGIN_MS}ms, and setTimeout clamps a larger delay to 1ms), got ${String(resolved.graceMs)}`)
  }
  ...
  for (const key of ['maxLogBytes', 'maxValueBytes'] as const) {
    ...
    if (!Number.isInteger(resolved[key])) {
      throw new Error(`dsh-code-runtime-python: config.${key} must be a positive integer (the child reads it as an int, so a float diverges from the host), got ${String(resolved[key])}`)
    }
    ...
    if (key === 'maxLogBytes' && resolved[key] < MIN_LOG_BYTES) {
      throw new Error(`dsh-code-runtime-python: config.maxLogBytes must be at least ${MIN_LOG_BYTES} (a smaller budget cannot serialize the truncation marker itself, so a marker-only truncated run would return more than the configured cap), got ${String(resolved[key])}`)
    }
  }
  ...
  if (budgetableBytes <= 0) {
    throw new Error(`dsh-code-runtime-python: config.addressSpaceMb must exceed the ${INTERPRETER_BASELINE_BYTES}-byte interpreter baseline with room for the output budgets, so the child has address space left to build and encode them; got ${String(resolved.addressSpaceMb)} MiB (${addressSpaceBytes} bytes)`)
  }
...
export function resolveInterpreter(bin: string): string {
  ...
  if (pythonBin === undefined) {
    ...
    throw new Error(`dsh-code-runtime-python: config.pythonBin ${JSON.stringify(bin)} ${explicit ? 'is not an executable regular file' : 'does not resolve on PATH'}`)
  }
