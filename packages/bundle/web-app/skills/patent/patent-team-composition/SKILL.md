...
| 制图员 | `illustrator` | 中立（流程） | 附图的唯一权威产出与核验：按案情出图（示意图/流程图/框图/层级图/状态图/电路图/曲线图/剖视图/时序图/外观设计视图排布，三维模型走结构线稿）、维护附图标记表（细则21.2 双向一致的唯一标记源）、标号回填、图文双向一致性核验、形式要件核验（图号正下方/除必需词语外无注释/色彩与落版尺寸）；答复、补正、复审场景的附图修改执行 | 撰写、答复、补正、复审 | `patent-illustrator`（附图文件/附图标记表/图文一致性/形式要件核验） |
...
成员：检索员、撰写员、对立审查员、技术专家、申请人代理、制图员、文档专员。目标：新申请文件（权利要求书+说明书+附图与附图标记表）并以正式文书交付。
...
成员：检索员、撰写员、对立审查员、技术专家、申请人代理、制图员、文档专员。撰写团队可直接延续（同成员不重建）。目标：OA 答复并以正式意见陈述书交付。
...
成员：撰写员、形式审查员、制图员、文档专员。目标：补正通知书答复（形式缺陷）或主动补正，以正式补正书交付。

| 任务 | 主题 | 负责人 | 依赖 |
|---|---|---|---|
| t1 | 补正通知书解析：形式缺陷清单 | 形式审查员 | — |
| t2 | 补正书与替换页起草 | 撰写员 | t1 |
| t2a | 附图补正执行与形式要件核验：替换页对应图幅、图号位于附图正下方、除必需词语外无注释、落版尺寸与色彩 | 制图员 | t2 |
| t3 | 缺陷消除核验：逐项对照清单 | 形式审查员 | t2, t2a |
...
成员：检索员、撰写员、对立审查员、申请人代理、制图员、合议组、文档专员。目标：驳回决定复审（3 个月期限），以正式复审请求书交付。

| 任务 | 主题 | 负责人 | 依赖 |
|---|---|---|---|
| t1 | 驳回决定解析与复审可行性评估（期限/恢复窗口核验） | 申请人代理 | — |
| t2 | 复审请求书与修改方案起草 | 撰写员 | t1 |
| t2a | 附图修改执行与标号一致性核验（附图修改受 A33 约束） | 制图员 | t2 |
| t3 | 请求书评审：争辩强度 + 法条核验 | 对立审查员 | t2, t2a |
...
无效与诉讼包不纳入制图员：这两类场景的附图是比对与解释材料（专利法第五十九条第一款用于解释权利要求），不是产出物；确需重绘图面时由 captain 在 `maxMembers` 上限内临时 `patent_teams_add_member(role="illustrator")`。

## 创建序列

1. `patent_teams_create`：name 用 `patent-team-<案卷号>`，description 写明案卷、作业类型（立案/撰写/答复/补正/复审/无效/诉讼）与案卷根目录。
2. 按场景角色包依次 `patent_teams_add_member`（先中立/通用角色，后对抗双方，最后裁判；制图员与文档专员为流程中立角色，随中立角色一并加入——制图员负责附图与附图标记表，文档专员负责收口前的正式文档输出）。
...
成员 persona 已内置作业纪律（检索先于结论、`source_path` 溯源、法条核验、逐特征比对、免责声明、期限走工具、受阻上报不重试、提交前自检）与角色契约。派工文本因此只写本案事实与验收要求，**不复述纪律与契约**：复述既重复占用双方上下文，又会让成员把常设规则误读成"本次任务的要求"。每一条任务描述按五段写，缺段即视为派工不完整：

- **目标**：这条任务产出什么、给谁用（一句话）。
- **输入**：必读材料的具体路径（案卷背景、交底书、对比文件、上游产出）与要核的法条／判例。
- **产出**：落盘路径与文件名（按 `patent-workspace-layout` 的目录约定），以及必须包含的字段。
- **验收**：通过标准（字段齐、来源可溯源、自检已跑）与自检工具。
- **禁止**：本条任务特有的越界（如「不改实体结论」「不代对方起草」），并附「受阻即上报，不重试、不绕过工具」。

两条配套纪律：

- 通道：派工、追问、催办一律走 `patent_teams_send_message`，状态变更走 `patent_teams_claim_task` / `patent_teams_update_task`。插件已对成员隐藏通用 `send_message`，派工文本不必再交代通道；captain 自己也不要用它——该通道的消息不进团队邮箱、不产生 `patent-teams/*` 事件，看板看不到。
- 收敛：同一条纪律连续两次被成员违反，先检查任务描述与 `role` 是否错配（角色契约才是纪律的载体），再补派工文本。不要把纪律在派工文本里越写越长。
