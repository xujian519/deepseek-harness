...
it('GraphBuilder: 编译校验（保留哨兵/未注册节点/非法 maxSteps）', () => {
  const b = new GraphBuilder()
  expect(() => b.addNode(GRAPH_END, node('x', 1))).toThrow(GraphEngineError)
  expect(() => b.addNode('', node('x', 1))).toThrow(GraphEngineError)
  b.addNode('a', node('x', 1))
  expect(() => b.addEdge('missing', 'a')).toThrow(GraphEngineError)
  expect(() => b.setNodePolicy('missing', {})).toThrow(GraphEngineError)
  expect(() => b.compile('missing')).toThrow(GraphEngineError)
  expect(() => b.compile('a', 0)).toThrow(GraphEngineError)
  expect(() => b.compile('a', 1.5)).toThrow(GraphEngineError)
})

it('engine: 顺序边串行执行', async () => {
...
it('engine: 并行超步（fan-out 汇聚）+ LWW 确定性', async () => {
  const builder = new GraphBuilder()
  builder
  ...
  expect(result.state.k).toBe('p2')
  expect(result.state.done).toBe(true)
})

it('engine: 条件边扇出（router 返回多目标）', async () => {
  const builder = new GraphBuilder()
  builder
    .addNode('entry', node('start', true))
  ...
  expect(result.state.side).toBe('right')
  expect(result.state.end).toBe(true)
})

it('engine: 条件边受控循环 + maxSteps 护栏', async () => {
  let counter = 0
...
it('engine: maxSteps 耗尽（死循环护栏）completed=false', async () => {
  const builder = new GraphBuilder()
...
it('engine: 节点失败 → 降级标记，其余继续（fail-open 默认）', async () => {
  const builder = new GraphBuilder()
  builder
  ...
  expect(result.completed).toBe(true)
  expect(result.state.done).toBe(true)
  expect(isDegraded(result.state, 'failing')).toBe(true)
  expect(result.degraded.length).toBe(1)
  expect(result.degraded[0]?.reason).toBe('node_failed')
})

it('engine: failFast=true 节点失败立即终止', async () => {
  const builder = new GraphBuilder()
  builder
    .addNode('failing', async () => {
...
it('engine: GraphInterruptError 暂停（completed=false，带 interrupted 信息）', async () => {
  const builder = new GraphBuilder()
  builder
  ...
  expect(result.state.analysis).toBe('完成')
  expect(result.state.never).toBeUndefined()
})

it('engine: onSuperStepStart 钩子（检查点预留）', async () => {
...
it('engine: sideEffect 节点 delta 不合并', async () => {
  const builder = new GraphBuilder()
  builder
  ...
  const result = await graph.run({})
  expect(result.state.flag).toBeUndefined()
...
it('engine: 节点访问深拷贝快照（并行不互相污染）', async () => {
  ...
  const result = await graph.run({})
  expect(result.state.saw).toBe('seed')
})

it('engine: resume 需要 GraphCheckpoint（未接线时行为由 checkpoint 层定义）', async () => {
  const builder = new GraphBuilder()
