...
Surface events (`user/message`, `assistant/message`, `tool/result`) must declare how they join the ordered surface; raw chunks, boundaries, and other log-only events never produce a message. Surface events require the `surfaceOp` intent; non-surface events accept an optional `{ ignorable: true }` marker so out-of-repo plugin telemetry can be safely skipped by readers that do not know its type.
