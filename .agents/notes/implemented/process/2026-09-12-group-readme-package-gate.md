...
- **`verify-group-readme-packages` is the owner.** It compares each group's `## Packages` (or `## 包`) section against the directories under `packages/<group>/` that hold a `package.json`, reporting one diagnostic per package the section does not list and one per row whose directory holds no package. It also gates both root package maps' `## Package groups` (or `## 包分组`) table against the group directories, reporting a group with no row, a group listed more than once, and a row naming a directory that holds no group README. It runs inside `doc-sync` as a quick leaf, so `pnpm run test:docs` and the fork CI documentation job execute it.
- **A row counts when it links the package's README.** The check reads link targets in the section's table rows and takes the directory the target names, so a row's link text, column count, and family order stay the author's choice. A `../` target points into another group and is ignored: that group's README owns the row, which is how `client/` links `../test-support/client-runtime/` and `fs/` links `../e2b/fs-e2b/`.
...
- **Generate the tables from the directory listing.** Rejected: each row carries a hand-written Role and, in most groups, the `ctx` key the package provides; the tables are also ordered by theme rather than alphabetically, so a generator either loses the prose or rewrites all 59 tables to satisfy itself.
...
`scripts/verify-group-readme-packages.spec.ts` covers both exported checks: the accepted group table and root group table, each omitted package, a package listed twice, rows spread across family tables, a satisfying row outside the Packages section, a `../` cross-group target, a duplicate root row, a group listed three times, a root row whose directory holds no group README, a root row missing, a group the section prose links without listing, the Chinese headings with `.zh.md` targets, and the missing-section cases. `scripts/run-gates.spec.ts` asserts the leaf stays in `doc-sync`. The repository passes with 118 group READMEs checked (59 groups × 2 languages); `pnpm run test:docs` and `pnpm run verify-translation-pairing` were re-run after the table edits.
