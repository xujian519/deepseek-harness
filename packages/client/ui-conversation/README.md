...
A target is active while shell selection names it or while at least one subscriber claims its source. Becoming active replaces that target from current Contexts once and then applies the incremental flushes; creating a source claims nothing. The shell shows one View at a time, so selecting another View releases the previous target: with no subscriber left it keeps its builder and snapshot but receives no further updates until it is selected or subscribed again.
...
View selection is deterministic: a registered persisted selection wins, otherwise registered `chat` wins, otherwise no View renders. It never chooses the first registered View. A peer plugin that must switch a session's active view programmatically — the per-session store handle is apply-local by design — goes through `ctx.conversation.setActiveView(sessionId, view)`: the session body registers its store-backed setter on the controller, and the method reports whether a setter is mounted yet so a caller can retry across the session-seat race. Shell phase combines Session lifecycle with the activity classification of the selected target; no other target-specific snapshot is read by the shell.

The shell reads the persisted View preference before rendering when a Session first binds or a cached Session becomes current, selects the registered preferred View or Chat fallback, and selects later tab or focus choices before committing them to the store. A blank Session still omits the `conversation.view` slot, and a View that nothing selects or subscribes to stays unassembled.
