...
表层事件（`user/message`、`assistant/message`、`tool/result`）必须声明如何进入有序 surface；原始分片、边界与其他仅日志事件从不产生消息。surface 事件必须带 `surfaceOp` 意图；非 surface 事件接受可选的 `{ ignorable: true }` 标记，让不认识其类型的读取器可以安全跳过仓库外插件的纯信息遥测。
