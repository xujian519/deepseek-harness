...
| `generate_patent_figure` | drafting | Figure DOT builder + Graphviz `dot` CLI subprocess (Config.graphvizExecutable / figureOutputDir / dotFont); persists to the figure index (Config.figureIndexFile) |
| `add_patent_figure_references` | drafting | SVG annotation post-processing (matches `<text>`/`<tspan>` content, appends `(numeral)`) |
...
- **Figure generation scope** — `generate_patent_figure` renders single figures through the Graphviz `dot` CLI (no leader-line numerals: the numeral is embedded in the component label, e.g. `Processor (20)`); `semantic` color fills are allowed only when color carries technical content per CNIPA Guidelines Part I Chapter 1 §4.3 (2023 rev., `grayscale` is the default); `raw_dot`/`template` outputs carry no structured components or connections (search-index entries remain partial); multi-panel figures (FIG. 1A/1B) and cross-figure auto-number memory are not implemented (pass explicit `numerals` for cross-figure continuation). Graphviz is a system dependency — without it the tool fails loud with install guidance.
