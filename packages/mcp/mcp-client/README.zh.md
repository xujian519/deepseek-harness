...
当服务器在其 initialize 响应中提供 `instructions` 时，它们会注册为 `mcp:<serverName>:instructions` system-prompt 段落（order 155，工具指引带）。该段落每次组装都会重读当前代际，因此重连后返回不同 instructions 时无需重新注册即可生效；缺失或空值不贡献任何内容。通过 `surfaceInstructions: false` 关闭。
...
- **只桥接 MCP 的工具与服务器 instructions 能力**——Resources 与 Prompts 没有 harness 消费机制，暂缓实现。
