...
- `pre-push` runs `pnpm run typecheck`, which completes the Host lib phase, including generated Typert contracts, before the Client TypeScript check, and refuses a push whose destination is `master`, because trunk changes land through a pull request that runs the `master` ruleset's required checks first; `git push --no-verify` skips that refusal.
