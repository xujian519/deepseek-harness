...
- `afterEach` drops the fixed settle and keeps a single best-effort `unlinkSync`: Node unlinks the POSIX socket file when the closing server releases it and a Windows pipe name is not a file, so instrumenting the first version of this change showed `ENOENT` on all ten attempts of its retry loop — the loop waited 200 ms per case for a file that was already gone.
...
- **Wait for the socket file to disappear with a bounded retry.** Tried, then rejected: instrumenting the loop showed `ENOENT` on every attempt, because the closing server had already removed the file, so the wait only added 200 ms per case to the suite it was meant to speed up.
