...
A schema node type redaction cannot expand (`tuple`, and any future node type) throws instead of forwarding its value when a `role('secret')` field is reachable beneath it; with no reachable secret the value passes through like a scalar.
