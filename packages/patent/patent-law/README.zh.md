...
- **《专利法》的主题词已改用 2020 年修正编号。** 现有技术抗辩、不视为侵权、合法来源三组主题词原本按 2008 年修正的 62/69/70 条收录，现改到 2020 年文本承载它们的 67/75/77 条；62 条改按其 2020 年文本的主题（强制许可使用费）。工作流质量门禁的静态表原本也用同一组 2008 年编号，现已一并改到 67/75/77 条。
...
- **指南索引没有文本，其路径经过一次订正。** 《专利审查指南》按节路径索引、只有主题词，因此每条指南引用都报「未核验」，直到部署转录自己依赖的章节。这些路径取自本仓库自身的引用，其中三处指向的不是该节、甚至不是一节——「技术领域」挂在 2.2.1，而 2023 年修订把该节编为 2.2.2；第九章写作「2.0」，而该修订写作「2」；第一部分第一章 2.3 根本不是一节——已逐行改到它所指的节，并另按该节自身内容重写了五行主题词。主题词是否属于该节，仍只有转录后的节文本能核对。
...
- **自动门禁仍保留自己的表。** 专利 preset 已挂载本插件，模型会先跑 law_verify；工作流质量门禁的静态主题表与 `patent-rule` 的条号上限仍各自作决定。门禁的条号现在与本部索引在两部已转录法规上一致，但它收录的条文更少，其 `unknown` 判定会放行自己名下没有行的引用——第 69、70 条即属此列，本部索引把它们报为 `not-indexed`。改读本部索引仍属后续工作，因为门禁用自己那套五种判定作决定。
