...
挂载本选择器的组合要把它能挂载的四个包连同本包一起声明——两个 Host 后端与两个客户端面。判定在启动时作出，挂载通过运行时说明符完成、按组合应用自身的 base URL 解析，因此只写选择器一行的 `cordis.yml` 对 Loader 完整、对安装不完整：`verify-cordis-config` 会把要求扩展到这四个包，清单漏掉其中之一即报错。本包把同样四个声明为对等依赖，因为它一个都不 import。
