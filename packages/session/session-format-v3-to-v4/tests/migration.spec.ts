...
describe('V3 to V4 source preservation', () => {
  ...
  it('carries this repository\'s own V3 writer vocabulary into V4 by identity', () => {
    // These names come from the fork's V3 writer, not upstream's: `patent`,
    // `patent-teams`, and `self-evolve` plugins appended them non-ignorable, so
    // without their names in the released set every recorded patent-mode run
    // and self-evolve run would refuse migration, and an ignorable one would
    // be renamed to `plugin:<type>` and lose its consumers' fold.
    const names = [
      'agent/request-error',
      'patent-teams/member-added', 'patent-teams/member-removed', 'patent-teams/message-sent',
      'patent-teams/task-created', 'patent-teams/task-gated', 'patent-teams/task-updated',
      'patent-teams/task-validated', 'patent-teams/team-created', 'patent-teams/team-deleted',
      'patent/model-call', 'patent/plantask', 'patent/workflow-run',
      'self-evolve/commit', 'self-evolve/end', 'self-evolve/mined', 'self-evolve/proposed',
      'self-evolve/reflection', 'self-evolve/start', 'self-evolve/validated',
    ]
    const rows = names.map((type, seq): SessionFormatEvent => ({ type, seq, time: seq + 2, data: { note: type } }))
    const migrated = migrate(rows)
    expect(migrated.events).toEqual(rows)
    expect(migrated.events.map(event => event.type)).toEqual(names)
    expect(migrated.events.every((event, index) => event === rows[index])).toBe(true)
  })
