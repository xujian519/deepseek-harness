...
| State ownership | Sharing executable dependency graphs would let CLI and Desktop change each other's dsh, Cordis, plugin, or native-module versions, while two desktop processes could race on the same profile. | Electron acquires its process-lifetime single-instance lock before any profile access and exclusively owns `$DSH_HOME/profiles/desktop-runtime` plus its package-manager state. CLI and Desktop share supported product data under `$DSH_HOME`, but never executable packages, plugin activation, lockfiles, or `node_modules`. |
...
The [Office conversion provider](../../packages/document/office-to-pdf/README.md) carries the target’s declared native engine, or WASM when the kit declares no matching native target. Preparation rejects a missing target engine before packaging. Engine assets, licenses, and notices remain in the runtime dependency tree. That engine package ships beside ASAR under `resources/node_modules/@deepseek-ai/`, because its conversion helper is a separate process that starts the engine executable and reads the LibreOffice tree beside it as ordinary files; Node's resolution from inside `app.asar` reaches that directory, while an archive path reports no permission bits. Every other runtime package stays in `app.asar/dsh`.
