...
`dsh-better-sidebar` 为 dsh web GUI 提供类 VSCode 的右侧工作区——文件 explorer、editor、按会话隔离的终端、git 面板、side chat 会话、subagent 预览与内嵌浏览器——全部作用于当前打开的会话。文件、git 与终端操作都通过带围栏的宿主路由作用于当前会话的工作目录；切换会话即切换工作区。宿主半侧挂载 `/sidebar/*` 路由、node-pty 终端与默认关闭的 `terminal_*` 工具；浏览器半侧渲染面板并发布一个客户端服务，供注册侧边栏标签页与文件查看器。桌面组合把该行以停用状态插入（[卸载的决定](../../../.agents/notes/implemented/architecture/2026-09-15-desktop-unmounts-workspace-sidebar.zh.md)）；浏览器 `dsh web` 组合不挂载任何东西。
...
侧边栏靠一行 insert 挂载：加入该行的组合即会在会话旁得到面板，无需其他接线。桌面组合该行以停用状态插入；浏览器 `dsh web` 组合不挂载任何东西，因此想要该面板的部署需自行插入。
...
- **Git 探测预算固定** — 工作区容器回退扫描最多探测 200 个子目录、单个探测超过 5 秒即放弃、发现结果缓存 60 秒。三者约束的都是面板自身的探测行为（200 上限即 #369 的冻结修复），因此慢速或远端检出会报「未检测到仓库」而不是卡住；均不作为 `Config` 字段。

- **没有已发货组合挂载它**——桌面组合与浏览器 `dsh web` 组合都不挂载该面板；加入 patch 行就是启用，而不是设置项。
