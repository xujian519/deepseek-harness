...
本树已落地 `src/index.ts` FIXME 所要求的 `@deepseek-ai/dsh-timeout-guard` 改名；[改名台账](../../../.agents/notes/archived/architecture/2026-08-11-repository-naming-contract-and-rename-ledger.md) 仍把 `@deepseek-ai/dsh-tool-call-timeout-policy` 记录为既定名称，因此台账条目在本树中已陈旧。
