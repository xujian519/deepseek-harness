...
/**
 * Workflows that build or pack the workspace. Upstream bans `DSH_CLIENT_` from
 * these documents entirely — their builds must stay env-neutral at every step —
 * while the preview workflow may scope a `DSH_CLIENT_TITLE` to its own step.
 */
...
describe('client build environment', () => {
  ...
  it('keeps public client values out of workflow-wide environments', () => {
    for (const path of dshBuildWorkflows()) {
      ...
      expect(JSON.stringify(Reflect.get(document, 'env') ?? {}), path).not.toContain('DSH_CLIENT_')
      if (workspaceBuildWorkflows.has(path.split('/').pop() ?? '')) {
