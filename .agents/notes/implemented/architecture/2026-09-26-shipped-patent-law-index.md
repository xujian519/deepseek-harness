...
The guideline index's paths were checked the same way, and three of the eighteen did not name the section they claimed: 技术领域 was registered at 2.2.1 against a revision that numbers it 2.2.2, 第九章 was written 「2.0」 against a revision that writes 「2」, and 第一部分第一章 2.3 is not a section. Five further rows carried another section's or the whole chapter's labels — 2.3 held 说明书撰写要求 where the section is 说明书附图, and 4.1 held 审查意见 and 答复 where the section is 审查的文本. Each row was repointed at the section it names and its topics taken from that section's own content. The check used a locally extracted full text that is not shipped, so repeating it needs its own copy of the 2023 revision.
