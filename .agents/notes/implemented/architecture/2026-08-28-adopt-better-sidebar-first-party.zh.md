...
桌面 overlay（`apps/desktop-host/config/desktop.cordis.patch.yml`）自[重新挂载的决定](2026-09-10-desktop-workspace-sidebar-remount.zh.md)起重新挂载 `better-sidebar` 行，浏览器 `dsh web` 组合不挂载任何东西，因此想要该面板的浏览器部署需自行插入该行。
