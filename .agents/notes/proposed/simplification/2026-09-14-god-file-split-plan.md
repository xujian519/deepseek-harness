...
| `packages/typert/generator/src/analyzer.ts` | 2206 | The package analyzer over the extracted type graph | yes |
| `packages/experimental/code-runtime-python/src/index.ts` | 1180 | The plugin, the extracted config gates, and one oversized method | yes — config gates landed |
...
- `code-runtime-python` contains two `/* jscpd:ignore-start */` blocks whose comments declare a shape parallel to its sibling worker-thread backend: the constructor/teardown/run shape and the timer/abort/live-set block. Measured 2026-09-15 by removing each block's markers in turn: the first still suppresses a real clone (without it, 24 lines and 88 tokens match the sibling's constructor and teardown), while the second suppresses nothing — the two backends' timers, abort listeners, and live-run records no longer match token for token. So the second block's comment describes a parallel clone detection does not see, and the slice that moves its region decides whether to retire the markers. `fixture.ts` carries no such block; the package's only one is in `fixture-projections.ts`, which mirrors host timing without importing a target implementation.
...
- [Extracting the `Session` object and the publication observers](../../implemented/simplification/2026-09-15-session-object-extraction.md) (batch 3's third landed cut)
- [Extracting the trajectory ledger's row](../../implemented/simplification/2026-09-15-trajectory-row-extraction.md) (batch 3's fourth landed cut)
- [Extracting the analyzer's type graph](../../implemented/simplification/2026-09-15-analyzer-type-graph-extraction.md) (batch 3's fifth landed cut)
- [Extracting the python backend's load-time gates](../../implemented/simplification/2026-09-15-python-config-gates-extraction.md) (batch 3's sixth landed cut)
