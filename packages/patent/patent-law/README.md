...
- **The guideline index carries no text, and its paths were corrected once.** 《专利审查指南》 is indexed by section path with topics only, so every guideline citation reports 未核验 until a deployment transcribes the sections it relies on. The paths arrived from this repository's own citations, and three of them did not name the section they claimed — 技术领域 sat at 2.2.1 where the 2023 revision numbers it 2.2.2, 第九章 wrote 「2.0」 for a revision that writes 「2」, and 第一部分第一章 2.3 is not a section at all — so each row was repointed at the section it names and five topic lists were rewritten from that section's own content. Whether a topic belongs to its section, only the transcribed text can check.
