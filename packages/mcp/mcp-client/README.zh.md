...
| `surfaceInstructions` | `true` | 将服务器的 `instructions` 发布为 system-prompt 段落；设为 `false` 时仅桥接工具 |
...
每个成功连接返回的非空白指令保存在一个带服务器名称的段落中。未返回指令或指令仅含空白时，不向提示词添加文本。花括号保持字面值。替代连接仅在发现成功后发布其指令；释放或耗尽恢复预算时移除该段落。设置 `surfaceInstructions: false` 时完全不注册该段落，因此服务器指令不会进入提示词，而其工具仍然可用。
