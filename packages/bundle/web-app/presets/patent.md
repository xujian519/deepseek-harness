...
- `@deepseek-ai/dsh-patent-law` — law_verify: decides each law citation against the law index shipped in that package (《专利法》《专利法实施细则》 by article, 《专利审查指南》 by normalized section path), and reports an indexed article whose text has not been transcribed as 未核验 instead of accepting it. The shipped index carries article numbers, topic keywords, and the source recorded per entry only — no entry carries transcribed text yet — so a default deployment treats every citation as unverified, which is the point: the discipline that a citation must be verified against an official source is unchanged, and the tool no longer lets an unchecked article read as checked.
...
The model sees the Chinese patent-agent persona (professional identity, seven work disciplines, the standard workflow, the case-type routing table, and the output discipline with its mandatory disclaimer), the patent plan-mode section, the fifteen preset skills, and the patent tools — including `law_verify`, whose description states that an unverified article is never reported as checked — plus the standard coding tools. The persona requires verify-before-cite (web_fetch on every fact), separate comparison, per-feature comparison with citations, and a mandatory disclaimer on every analysis output.
...
- The shipped law index carries no transcribed article text, so its entries are all unverifiable today and `law_verify` reports every citation as 未核验. Transcribing the articles a deployment relies on from official sources — and re-recording each entry's `sourceDoc` / `verifiedOn` — is follow-up work; until then the tool narrows what must be looked up rather than replacing the lookup. The `专利法` index's 62/69/70 group also appears to carry pre-2020 numbering, recorded in the asset header.
