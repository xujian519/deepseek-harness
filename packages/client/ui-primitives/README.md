...
While a reply streams, `MarkdownText` parses incrementally: all but the trailing two blocks freeze as cached React elements and only the source tail re-parses per chunk, so per-chunk work tracks the tail instead of the whole reply. A reply that streams as one growing top-level block — a single paragraph, or a list that keeps taking new items — cannot freeze any of it, so every frame would re-parse all of it and cost would grow with the square of the reply; those frames instead pass a measured admission gate (`src/markdown/stream-frame-gate.ts`): a frame above a quarter-frame budget buys the following frames a cool-down of three times its own measured cost, which holds the streaming render to a quarter of the main thread, and the message component retries the held-back frame on a timer so a paused reply still catches up. Frames inside the budget, and replies under four kilobytes, are admitted unchanged. A final unclosed top-level fence keeps its parsed code node and sends only the last completed line plus the current partial line through the same GFM grammar; a closing fence or ambiguous parse returns to the ordinary tail path. Highlighting likewise resumes from saved Shiki grammar state and publishes only newly completed lines plus the mutable tail. `CodeBlock` seals completed lines into fixed-size React groups, reuses earlier groups, and retains the whole highlighted tree across settlement when code and language are unchanged. The settled full parse still resolves references that crossed the freeze boundary.
...
- **A long unclosable block trails the stream by a frame or two** — a reply that streams as one growing top-level block cannot freeze, so its frames pass the measured admission gate and the visible text can sit behind the arrival by up to four times one frame's parse cost. Nothing is dropped: the retry timer delivers the held-back text once the cool-down ends, and settling renders the full text immediately.
