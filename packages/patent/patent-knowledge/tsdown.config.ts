...
/**
 * patent-knowledge ships THREE runtime entries: the plugin (index), the
 * invariant companion (invariant), and the patent-knowledge-install CLI (bin).
 * Each is built as a self-contained file with code splitting disabled, so the
 * shared install/engine modules are inlined into every entry instead of
 * emitting a hashed chunk the published files list would miss (same pattern as
 * dsh-sdk-jsonrpc-demo). Declarations come from tsc -b (dts: false).
 */
