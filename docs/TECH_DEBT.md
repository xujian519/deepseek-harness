...
- **共享 `errorMessage` 的硬化候选**(2026-09-12 记录,未实施):其 `Error` 分支原样返回 `value.message`,而该属性在运行期可为任意值、返回类型却声明为 `string`;`code-runtime-python` 正因这条线才保留自己的实现。改为 `String(value.message)`(对 `string` 输入等价)即可消解,但会改变「恶意 `Error.message`」的现返回值,需独立 PR 与测试。
