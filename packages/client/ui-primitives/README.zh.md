...
回复流式输出期间，`MarkdownText` 增量解析：除末尾两个块外全部冻结为缓存的 React 元素，每个分片只重新解析其后的源文本尾部，因此每分片的工作量跟随尾部而非整个回复。若回复整体是一个不断增长的顶层块（单个段落，或持续追加条目的列表），它任何部分都无法冻结，于是每一帧都要重新解析全文、成本随回复长度的平方增长；这类帧改为经过实测准入闸门（`src/markdown/stream-frame-gate.ts`）：超过四分之一帧预算的帧会为其后的帧换取一段等于自身实测成本三倍的冷却，从而把流式渲染压在四分之一主线程以内，消息组件再用定时器重试被扣留的帧，因此暂停的回复也能补齐。预算之内的帧、以及四 KB 以下的回复，一律照常准入。末尾的顶层未闭合 fence 会保留已解析的 code node，只把最后一个已完成行与当前未完成行交给同一套 GFM grammar；闭合 fence 或有歧义的解析会回到普通尾部路径。高亮同样从保存的 Shiki grammar state 续接，并只发布新完成行与可变尾部。`CodeBlock` 把已完成行封入固定大小的 React 分组、复用更早的分组，并在代码与语言未变化时跨定稿保留整棵高亮树。定稿时的全量解析仍会解析跨过冻结边界的引用。
...
- **无法闭合的长块会比流落后一两帧**：整体为一个不断增长的顶层块的回复无法冻结，其帧要经过实测准入闸门，因此显示文本最多落后到达内容一个帧解析成本的四倍。不会有内容丢失：冷却结束后重试定时器会交付被扣留的文本，定稿渲染则立即显示全文。
