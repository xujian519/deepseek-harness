...
async function main(): Promise<void> {
  ...
  const quitConfirmation = new DesktopQuitConfirmation({
    ...
    ...(process.platform === 'win32' ? { icon: nativeImage.createFromPath(quitDialogIconPath) } : {}),
  })
  ...
  const createMainWindow = (): BrowserWindow => {
    ...
    window.on('close', (event) => {
      ...
      if (!shouldHideOnClose(quitting || shuttingDown || sessionEnding, tray !== undefined)) return
      event.preventDefault()
      if (updateDialog.isOpen) { updateDialog.focus(); return }
      const hide = (): void => {
        if (!quitting && !shuttingDown && !sessionEnding && !window.isDestroyed()) hideMainWindow(window)
      }
