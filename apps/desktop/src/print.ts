 ...
 * dialog. The HTML arrives from the renderer (the harness web UI, custom
 * protocol origin) — the same trust class as opening the file in the system
 * browser; scripts in the document execute inside the hidden window, so
 * callers must only feed their own session's produced files.
 */
