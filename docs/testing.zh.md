...
- **构建产物车道**（`pnpm run test:built`；必需的 fork PR 门禁 `node 24 / built artifacts`）：车道先构建，再运行「未构建的单元车道会静默跳过」的那批套件，仍未产生任何通过的套件按名字告警（[车道规则](../scripts/built-artifact-suites.ts)）。
