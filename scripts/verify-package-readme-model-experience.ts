...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-todo-board': { kind: 'none', reason: 'Read-only view over an existing session projection; registers no tool, prompt section, or session event.' },
  ...
  'packages/client/ui-plugin-market': { kind: 'none', reason: 'Browser-side discovery projection; registers nothing model-facing.' },
  ...
  'packages/host/plugin-market': { kind: 'none', reason: 'Catalog discovery and managed install seam; registers no prompt, tool schema, or model request.' },
  'packages/api/plugin-market-controller': { kind: 'none', reason: 'Host Remote owner projecting a read-only seam; registers no prompt, tool, or session event.' },
  ...
  'packages/client/ui-open-in-app': { kind: 'none', reason: 'Browser-side split button opening the workspace directory for a human; registers nothing model-facing.' },  'packages/bundle/base': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
