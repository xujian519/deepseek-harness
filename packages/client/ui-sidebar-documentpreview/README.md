...
- **Packed-preview budgets are fixed** — one document packs at most 4 MiB per asset, 32 MiB in total, and 64 assets; the packer runs next to the preview inside the browser, so these bound the client's own memory and an oversized document is reported rather than configured.
