...
[`dsh-base`](../packages/bundle/base/README.md) is the shared first layer of the `web`, `headless`, `desktop`, `sdk`, and `acp` profiles: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application, [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server, [`dsh-sdk-app`](../packages/bundle/sdk-app/README.md) adds the SDK JSON-RPC server, and [`dsh-acp-app`](../packages/bundle/acp-app/README.md) adds the automation-only ACP server. The desktop surface is not a bundle; the Electron app layers it over `dsh-web-app` via `dsh-desktop-host`. [`dsh-sdk-minimal`](../packages/bundle/sdk-minimal/README.md) is the deliberate exception: one bundle owns its complete explicit SDK tree without `dsh-base`.
...
The [Electron desktop application](../apps/desktop/README.md) carries its exact dsh production runtime in signed application resources and exclusively owns the reserved `$DSH_HOME/profiles/desktop`. It adds no listener: the desktop composition opens neither a Web server nor a loopback port, and shares supported product data with the CLI profiles without sharing their executable packages, plugin activation, lockfiles, or package-manager state. Its README owns the transport, profile-layout, and package-transaction detail.
