...
class SystemdScopeOwner implements BoundProcessOwner {
  ...
  private async rangeActive(): Promise<boolean> {
    ...
    if (result.status === 0) {
      ...
      // A terminated range whose cgroup is empty is a leftover the manager
      // never observes, so release it before consulting residue state.
      ...
      if (!this.direct.running()
        && existsSync(this.startup.files.requestPath)
