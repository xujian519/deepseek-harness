...
| [`src/send-lifecycle.ts`](src/send-lifecycle.ts) | Ownership of the send holding the terminal slot: its deadline, cancellation listener, interrupt handoff, and provider write |
...
The readiness evidence lives on the session, which collects it and judges with it: `promptSeen`, `promptTextSeen`, and the retained `promptTail` for the private prompt marker; `lastOutputAt` for output silence; `shellPgid` for the foreground group bash owns, learned once when the first marker arrives; and `initializing`, which makes the exact-wait and silence tiers require output while the startup send is still running. `resetReadinessEvidence` drops the marker and output evidence at admission and again before a provider write that carries input, so a send is judged only on evidence its own write produced; `shellPgid` and `initializing` outlive that reset. The poller owns arming alone: when the next poll runs, and whether one may be armed for the send that holds the slot.
