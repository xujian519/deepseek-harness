...
本包在 Web GUI 中渲染跨会话任务看板：会话视图环里与 Chat、Trajectory 并列的 `Board` tab。它把每个工作区会话的最新整张待办列表（由 `todo/write` 事件写入的 `todosLatest` 会话投影）折叠为三列状态——待开始、进行中、已完成——让当前工作区所有会话的工作在一处可见。每张卡片带一个会话徽标；激活徽标会在会话区打开所属会话。空看板渲染三列的虚线幽灵预览。
