...
`)
    const failure = vi.fn()
    const host = new DesktopHostProcess(process.execPath, runtime, runtime, undefined, undefined, process.env, failure)
    try {
      await host.start()
...
`)
    ...
    roots.push(profile)
    const host = new DesktopHostProcess(process.execPath, runtime, profile, undefined, undefined, {
