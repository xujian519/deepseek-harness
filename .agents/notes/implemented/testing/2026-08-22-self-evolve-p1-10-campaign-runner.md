...
- The scaffold's honest status stays: no real 60-task campaign has been run in this repository. The runner is wired as a `campaign` CLI subcommand; its dry-run plan, git-pathspec exclusion, merge/verdict pure logic, and subprocess path (git/venv/pytest against temp repos with a stubbed verdict) are unit-tested. A keyed e2e (`pnpm run test:e2e`) drives one real task through the pipeline and self-skips without `DEEPSEEK_API_KEY`/`SELF_EVOLVE_E2E_MANIFEST`.
