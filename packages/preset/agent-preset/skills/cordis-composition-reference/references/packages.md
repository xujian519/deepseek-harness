...
| `@deepseek-ai/dsh-api-plugin-market-controller` | no | Remote owner for the open plugin-catalog discovery surface over the plugin-market seam |
...
| `@deepseek-ai/dsh-self-evolve-app` | no | The dsh self-evolve opt-in bundle: the self-evolve capability seam, provider, and tools as a profile patch layer |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-better-sidebar` | yes | DSH web plugin: a VSCode-like right sidebar (explorer / editor / terminal / git / browser), isolated per conversation session. Exposes a client service for other client plugins to register sidebar tabs and file viewers |
...
| `@deepseek-ai/dsh-client-synapse` | no | The Synapse session-map browser half for dsh web: the 对话/会话地图 view switch and the /synapse/ iframe bridge over the client session and workspace services |
...
| `@deepseek-ai/dsh-client-ui-document-studio` | no | Document-delivery studio view for the document agent preset: session-wide produced-file list, HTML/text preview, open and print actions, auto-switch when the session preset is document |
...
| `@deepseek-ai/dsh-client-ui-todo-board` | no | Cross-session todo board: a conversation view tab aggregating each workspace session's latest todo list into three status columns |
...
| `@deepseek-ai/dsh-prompt-cache` | yes | Prompt-prefix cache strategy for the DeepSeek Harness system-prompt assembly |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-desktop-directory-picker` | no | Electron-backed directory-picker provider for the DeepSeek Harness desktop shell |
| `@deepseek-ai/dsh-desktop-shell` | no | Electron-main bridge provider for the ctx.desktop capability seam |
| `@deepseek-ai/dsh-macos-tools` | yes | Model-facing macOS native tools (open/reveal, browser URLs, clipboard read/write, notifications, speech, app control) driven through system CLIs |

## document

| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-doc-template` | yes | Function plugin rendering the shipped Chinese document templates: YAML front-matter templates with a typed variable schema, validated resolution reporting residual placeholders and warnings, Markdown/HTML/DOCX renderers, and the list_doc_templates and render_doc_template tools |
| `@deepseek-ai/dsh-document-deliver` | yes | Structured deliverable registration for the document agent: document_deliver records the produced files, formats, and quality-gate state in the session log |
...
| `@deepseek-ai/dsh-tool-plugin-market` | no | Plugin catalog discovery tools: list catalog sources, search the bundled/online catalog, and preview a package before install |
...
| `@deepseek-ai/dsh-timeout-guard` | no | Tool-call timeout policy: a tools/execute wrapper that arms a per-tool deadline on exec.signal and returns TOOL_TIMEOUT when it wins |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-openviking` | yes | OpenViking context-database integration: auto-recall, session capture and commit, and the OpenViking tool surface |

## patent

| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-methodology` | yes | TRIZ reasoning methodology for the DeepSeek Harness: the 40 inventive principles and the 39x39 contradiction matrix as a model-facing tool and library |
| `@deepseek-ai/dsh-patent-data` | no | Patent data access seam: nuo-patent vendor mapping/search provider plus the ego-browser anti-crawl subprocess provider for the DeepSeek Harness |
| `@deepseek-ai/dsh-patent-deadline` | yes | Chinese patent statutory deadlines for the DeepSeek Harness: the period arithmetic of the Implementing Regulations (period start, corresponding day, rest-day roll-forward), delivery-date determination, and the patent_deadlines tool |
| `@deepseek-ai/dsh-patent-document` | yes | Patent document rendering for the DeepSeek Harness: template resolution, brand injection, and headless-Chrome PDF rendering via ctx.subprocess |
| `@deepseek-ai/dsh-patent-fees` | yes | Shipped Chinese patent fee index for the DeepSeek Harness: the fee items an application, a grant, or a procedure owes with each one's countable basis, a per-item record of the amount's official source and verification date, the fee-reduction and surcharge rules, and the patent_fees tool, which never presents an unverified amount as the total |
| `@deepseek-ai/dsh-patent-knowledge` | yes | knowledge.db query seam: case-law, legal, wiki-card, IPC, and knowledge-graph lookups plus the patent-knowledge:install data bootstrap for the DeepSeek Harness |
| `@deepseek-ai/dsh-patent-law` | yes | Shipped Chinese patent-law index for the DeepSeek Harness: law-article and guideline-section reference parsing, a per-law index recording each entry's source and verification date, citation verification that never silently passes an unverified article, and the law_verify tool |
| `@deepseek-ai/dsh-patent-rule` | yes | Patent compliance rule engine and output gates for the DeepSeek Harness: YAML rule packs, evaluation, EVI-011 evidence guards, and post-execute gating |
| `@deepseek-ai/dsh-patent-teams` | yes | Durable multi-agent team capability for patent workflow: captain-led teams, dependency-aware tasks, mailbox messaging, and an event-driven shared-task scheduler, surfaced as the patent_teams_* tools. |
| `@deepseek-ai/dsh-patent-tools` | yes | Model-facing patent tool set for the DeepSeek Harness: search, metadata, legal status, case/wiki/kg knowledge queries, claim-chart, drafting, evidence judgment, rule check, and the workflow/plan tools, ported from Sati |
| `@deepseek-ai/dsh-patent-workflow` | no | Patent execution pipeline: workflow/plantask state machine with HITL approval wiring for the DeepSeek Harness |
| `@deepseek-ai/dsh-tool-literature` | yes | Model-facing literature tools for the DeepSeek Harness: arXiv, OpenAlex, Semantic Scholar, and Crossref connectors |
| `@deepseek-ai/dsh-writing-patterns` | yes | Writing-pattern library for patent drafting and office-action replies for the DeepSeek Harness: a packaged YAML pattern corpus with lexical search and match, a deterministic <writing_skills> compiler, the four-dimension quality evaluation, a system-prompt section, and the query_writing_patterns tool |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-self-evolve-basic` | yes | Projection-driven idle-pressure evolution policy for the DeepSeek Harness self-evolve seam |
| `@deepseek-ai/dsh-self-evolve-benchmark` | yes | Benchmark-driven target evolution provider for the DeepSeek Harness self-evolve capability |
| `@deepseek-ai/dsh-tool-self-evolve` | no | Model-facing tools and prompt section for the DeepSeek Harness self-evolve capability seam |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-host-synapse` | yes | The Synapse session-map host half for dsh web: the /synapse canvas page/API and the committed-session projection store, mounted on the existing Web Server |
