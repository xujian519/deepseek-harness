...
Every pull request has a required `python-runtime` job in [CI](../../../../.github/workflows-disabled/ci.yml). It calls the shared [single-executable builder](../../../../.github/workflows-disabled/build-exe-for-python-sdk.yml) for `node24-linux-x64` without a path filter and participates in `all checks passed`. The called workflow builds the real executable, runs all keyless Python full-turn and direct-binary scenarios including both committed snapshots, builds the SDK and runtime wheels, installs them into a clean virtual environment, checks the executable and native addon's GLIBC requirements, and runs the installed wheels in a manylinux 2.28 container.
