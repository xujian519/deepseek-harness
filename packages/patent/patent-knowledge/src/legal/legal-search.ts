...
import { LAWS_DB } from '../shared/schema-versions.ts'
...
import { buildLawSearchSql, LAW_SEARCH_COLUMNS, LAW_SEARCH_COLUMNS_FTS } from './sql.ts'
import { dedupeByLawName } from './dedupe.ts'
import { toRecord, toSearchResult, type LawRow } from './row-mapper.ts'

export { extractLawKeywords } from './keywords.ts'
...
export class LegalSearchEngine implements LegalSearchSource {
  ...
  private searchFtsWithQuery(query: string, options: LegalSearchOptions, limit: number): LawRow[] {
    ...
    if (!options.level && !options.category) {
      const ftsSearch = this.stmtSearchFts
      if (ftsSearch === null) {
        // 不可达：FTS 入口（search）已确认 hasFts && !ftsDegraded，prepare 失败会置 ftsDegraded。
        rows = []
      } else {
        rows = ftsSearch.all(query, limit * 3) as LawRow[]
      }
    } else {
