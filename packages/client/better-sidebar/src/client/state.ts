...
export function makeDefaultState(width = PANEL_DEFAULT, panelOpen = true, seed: DefaultSeed = 'editor-home'): SidebarState {
  ...
  if (seed === 'editor-home') {
    ...
    leaf.active = (leaf.tabs[0] as SidebarTab).id
  }
...
export function setTabPin(
  ...
  const walk = (node: SplitNode): SplitNode => {
    if (node.kind === 'leaf') {
      ...
      const oldTab = node.tabs[idx] as SidebarTab
  ...
  const floats = floatIdx < 0 ? state.floats : (() => {
    const oldFloat = state.floats[floatIdx] as FloatWindow
...
export function resizeSplit(node: SplitNode, splitId: string, index: number, delta: number): SplitNode {
  ...
  if (node.id === splitId) {
    ...
    const left = Math.min(0.92, Math.max(0.08, (sizes[index] as number) + delta))
    const right = Math.min(0.92, Math.max(0.08, (sizes[index + 1] as number) - delta))
    sizes[index] = left
    sizes[index + 1] = right
    return { ...node, sizes }
  }
...
export function raiseFloat(state: SidebarState, floatId: string): SidebarState {
  ...
  floats.push(raised as FloatWindow)
  return { ...state, floats }
}
