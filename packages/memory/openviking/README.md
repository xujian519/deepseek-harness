...
- **Web status card deferred** — the settings form is live (the entry's volatile fields render in the Web UI settings page and every write is validated against the Config schema, endpoint pattern included), and `GET /openviking/status` serves health and queue JSON, but the browser status card (a client `ui-*` plugin with client-build registration) is not yet wired into a client slot. Status is currently reachable through the route, the `memqueue` tool, and the CLI.
