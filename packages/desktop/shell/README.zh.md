...
- **重连退避默认值位于客户端选项** — `BridgeClient` 默认 500 ms 基数、5 s 上限、10 次尝试；桌面壳是唯一构造点且原样采用，因此该策略由构造客户端的部署声明，而不由插件 `Config` 字段声明。
