...
- **`bash` 工具不采用 `timeout-guard` 预算**：根据[工具调用 timeout-policy Agent Note](../../../.agents/notes/implemented/architecture/2026-07-07-tool-call-timeout-policy.zh.md)，它保留由执行器持有的 `BASH_TIMEOUT` 路径。
