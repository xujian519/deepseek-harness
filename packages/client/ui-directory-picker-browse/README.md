...
Mount this plugin alongside `ui-workspace` and the host backend [`dsh-host-directory-picker-browse`](../../host/directory-picker-browse/README.md); one cordis.yml row then composes the whole browse picking interaction. When a workspace flow opens a directory request, the user sees the in-app dialog: a header with the path breadcrumb and an editable path zone, then a single full-width level until a row is selected, after which the row splits into level and children columns. A desktop shell's sandboxed preload can contribute a native folder chooser (`window.__DSH_DESKTOP_PICK_DIRECTORY__`) and an optional path-validation gate (`window.__DSH_DESKTOP_VALIDATE_DIRECTORY__`); when present, a quiet icon button beside the show-hidden toggle opens the system chooser, and every opened path — native or in-browser — runs through the validation gate before the owner adopts it.
