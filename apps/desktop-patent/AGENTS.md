...
The desktop profile is `desktop-patent`: `dsh --profile desktop-patent`. It is a Cordis bundle layered over `dsh-base` plus the web-app surface, with desktop-specific host plugins mounted through `packages/bundle/desktop-app/cordis.patch.yml`.
...
- Start dsh with `--profile desktop-patent --port 0`, discover the bound URL from the `dsh web:` readiness line on stdout, then load the renderer URL.
