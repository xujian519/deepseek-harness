...
This decision supersedes the package-name row of the ledger (`@deepseek-ai/dsh-timeout-policy` → `@deepseek-ai/dsh-tool-call-timeout-policy`) in the [repository naming contract](../../archived/architecture/2026-08-11-repository-naming-contract-and-rename-ledger.md) and the package-name rationale in the [tool-call timeout policy](2026-07-07-tool-call-timeout-policy.md) — names only. The timeout mechanism, the `TOOL_TIMEOUT` classification, the `tools/execute` wrapper semantics, the `guard/` home, and the `@deepseek-ai/dsh-timeout` deadline library are unchanged; the 2026-07-07 note remains the owner of the mechanism. The package-regrouping note records the `guard/` group inventory and the old `timeout/` merge as history.
