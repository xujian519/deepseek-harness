...
The application menu carries the platform `editMenu`, `viewMenu`, and `windowMenu` roles beside the shell-owned entries, so system copy/paste/undo accelerators and right-click editing work against the focused renderer. External `http(s)` links opened by the renderer hand off to the OS browser through `shell.openExternal`; the shell never opens a second `BrowserWindow`.
...
4. Plugin add, update, and remove operations use bundled pnpm and Desktop-owned package-manager state. Reserved host packages must be peers; nested copies and aliases of shared packages fail validation. The comparison admits prerelease versions, so a prerelease Host release satisfies a prerelease peer range that covers it. Ordinary plugin dependencies must resolve inside the profile.
